...
async function loadAIHistory() {
    ...
    // Only fetch if empty or refreshing (optimization: could check state cache)
    // But for simplicity we fetch to ensure sync.
    try {
        if (!state.user) {
        ...
        const response = await fetch(`${API_BASE}/recipes?is_ai_generated=true&sort=novelty`, {
             headers: getAuthHeader()
        });
        ...
        if (recipes.length === 0) {
            list.innerHTML = '<p style="grid-column: 1/-1; text-align: left; color: #888;">No AI recipes yet. Try generating one!</p>';
            // If empty, force exit edit mode
            if (isHistoryEditMode) toggleHistoryEditMode(false);
            return;
        }
        ...
        recipes.forEach(r => {
            ...
            // If Edit Mode, disable main click and show delete button
            if (isHistoryEditMode) {
                card.onclick = null; // No navigation in edit mode
